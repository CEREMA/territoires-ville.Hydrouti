...
Close #lhFicDbf1
End Sub
Public Function verif_regime(ByVal Q As Double, ByRef canal As conduite) As String
' q en m3/s
Dim regim As String
Dim betam As Double
Dim qv As deb_vit
...
With edo_r
    ...
    .Qdev = Qdev
End With
sres = sres + Chr(13) + Chr$(10) + " Débit dans la conduite étranglée " + Str(Round(Qav, 3)) + "m3/s"
sres = sres + Chr(13) + Chr$(10) + " Débit déversé : " + Str(Round(Qdev, 3)) + " m3/s"
...
If a <> 0 And l <> 0 And p <> 0 And amontana <> 0 And bmontana <> 0 Then
...
vst = 0#
   'début hydrogramme
    If DM <> 0 And dt <> 0 Then
        HT = dt * amontana * (dt) ^ bmontana
        HM = DM * amontana * (DM) ^ bmontana
...
'End If
xavp = tr.Absava
yavp = tr_res.zphe_ava
xamp = tr.Absamo
yamp = tr_res.zphe_ava + ((tr.conduit.Longueur) * ppiezo)
